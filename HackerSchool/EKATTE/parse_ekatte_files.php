...
$stmt = $conn->prepare("INSERT INTO selishta (kmetstvo, name, center, document) VALUES (?, ?, ?, ?)"); //selishta
//$param_types = "sissii"; // oblasti
//$param_types = "sisiii"; // obshtini
$param_types = "ssii"; // selishta
...
for ($row = 2; $row <= $highestRow; $row++){ 
	...
	$sql = "SELECT kmetstvo FROM selishta WHERE kmetstvo = '{$rowData[0][0]}'";  // oblasti - oblast, obshtini - obshtina, selishta - kmetstvo: 	  
	if ($result = $conn2->query($sql)) {
		if($result->num_rows <= 0) {
			$conn2->close(); 
			$stmt->bind_param($param_types, $rowData[0][0], $rowData[0][1], $rowData[0][2], $rowData[0][3]);
